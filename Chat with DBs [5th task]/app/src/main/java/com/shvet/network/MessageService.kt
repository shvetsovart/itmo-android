...
class MessageService : Service() {
    ...
    private fun postImageMessage(intent: Intent) {
        Thread {
            ...
            val fileName =
                "${System.currentTimeMillis()}" + contentResolver.getFileName(imageUri)
            ...
            Thread {
                try {
                    if (db!!.messageDao().getAll().isNotEmpty() && messageList.isEmpty()) {
                        ...
                        messageList.addAll(db!!.messageDao()
                    ...
                    if (unprocessedImages.isNotEmpty()) {
                        if (!isDelayedImageProcessingRunning) {
                            mainHandler.post {
                                Toast.makeText(
                                    applicationContext,
                                    "Loading messages...", Toast.LENGTH_SHORT
                                ).show()
                            }

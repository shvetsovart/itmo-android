...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        attachFileButton = findViewById(R.id.attach_button)
    ...
    private fun updateRecyclerView(recyclerView: RecyclerView,
        ...
        recyclerView.apply {
            ...
            adapter = MessageAdapter(messageList) {
                if (it.msgData.bitmapImage != null) {
                    val intent = Intent(this@MainActivity,
                        ImageFullResActivity::class.java)

                    intent.putExtra("id", it.id.toString())
                    startActivity(intent)
                } else if (it.msgData.msgText != null) {
                    Toast.makeText(context, "It's a text message from ${it.from}",
